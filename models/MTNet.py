...
class Encoder_Decoder(nn.Module):
    def __init__(self,in_c):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(in_c, in_c,kernel_size=5,stride=4,padding=1,bias=False),
            nn.BatchNorm2d(in_c),
            nn.LeakyReLU(),
        )
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(in_c, in_c,kernel_size=5,stride=4,padding=1,output_padding=1,bias=False),
            nn.BatchNorm2d(in_c),
            nn.LeakyReLU(),
        )
    def forward(self, X,Y):
...
class TransBlock(nn.Module):
    def __init__(self, hs_band):
        ...
        self.net2 = GSNet(hs_band,pan_band)
        self.itrator_num = 5
        self.t1 = nn.Parameter(torch.tensor(0.1))
        self.t2 = nn.Parameter(torch.tensor(0.1))
    def forward(self, Y,Z):#Y LRMS Z PAN
        X0 = F.interpolate(Y,scale_factor=4,mode='bicubic',align_corners=False)
        X = X0
