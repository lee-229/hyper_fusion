...
import math
from models.mytransformer import SwinModule
from model import conv
from model import CALayer
...
class RWDB(nn.Module):
  ...
  def forward(self, x):
    ...
    out = out + x
    return out
class RWDB_new(nn.Module):
  def __init__(self, nChannels, nDenselayer, growthRate):
    super(RWDB_new, self).__init__()
    nChannels_ = nChannels
    modules = []
    self.swin = nn.Sequential(SwinModule(in_channels=nChannels, hidden_dimension=nChannels, layers=2,
    ...
    for i in range(nDenselayer):
        ...
        nChannels_ += growthRate
    self.dense_layers = nn.Sequential(*modules)
    self.conv_1x1 = nn.Conv2d(nChannels_+nChannels, nChannels, kernel_size=3, padding=1,stride=1, bias=True)
    # self.conv9 = nn.Conv2d(nChannels, nChannels, 9, 1, 4)
    # self.conv7 = nn.Conv2d(nChannels, nChannels, 7, 1, 3)
    # self.conv5 = nn.Conv2d(nChannels, nChannels, 5, 1, 2)
    self.relu = nn.LeakyReLU()
  def forward(self, x):
    ...
    # out7 = self.relu(self.conv7(x))
    # out9 = self.relu(self.conv9(x))
    out = self.conv_1x1(torch.cat((out1, out2), 1))
    out = out 
    return out
...
class MDANet(nn.Module):
    def __init__(self,input_channel):
        ...
        Dim=[1, input_channel+1, input_channel]
        depth=1
        nDenselayer=4
        nFeat=64
        growthRate=32
    ...
    def forward(self, x,y): #x PAN y LRHS_UP
...
# version 2 use RWDB_new block*1 3*3conv
# class MDANet(nn.Module):
#     def __init__(self,input_channel):
#         super(MDANet, self).__init__()
#         Dim=[1, input_channel+1, input_channel]
...
#         for i in range(depth):
#             block1_1.append(RWDB_new(nFeat, nDenselayer, growthRate))
            
#             block2_1.append(RWDB_new(nFeat, nDenselayer, growthRate))
          
#             block3_1.append(RWDB_new(nFeat, nDenselayer, growthRate))
...
#         self.branch_out_1 = RWDB_new(nFeat, nDenselayer, growthRate)
#         self.branch_out_2 = RWDB_new(nFeat, nDenselayer, growthRate)
...
# version 3 use RWDB_new block*2
# class MDANet(nn.Module):
#     def __init__(self,input_channel):
#         super(MDANet, self).__init__()
#         Dim=[1, input_channel+1, input_channel]
...
#         for i in range(depth):
#             block1_1.append(RWDB_new(nFeat, nDenselayer, growthRate))
#             block1_2.append(RWDB_new(nFeat, nDenselayer, growthRate))
#             #block1_3.append(RWDB(nFeat, nDenselayer, growthRate))
#             block2_1.append(RWDB_new(nFeat, nDenselayer, growthRate))
#             block2_2.append(RWDB_new(nFeat, nDenselayer, growthRate))
#             #block2_3.append(RWDB(nFeat, nDenselayer, growthRate))
#             block3_1.append(RWDB_new(nFeat, nDenselayer, growthRate))
#             block3_2.append(RWDB_new(nFeat, nDenselayer, growthRate))
#             #block3_3.append(RWDB(nFeat, nDenselayer, growthRate))
...
# # # version 4 use RWDB_new block*1 + sperctral attention
# class MDANet(nn.Module):
#     def __init__(self,input_channel):
#         super(MDANet, self).__init__()
#         Dim=[1, input_channel+1, input_channel]
...
#         self.CA = CALayer(input_channel, 4, bias=True)
#         self.convlast = conv(192, input_channel, 1, True)
#     def forward(self, x,y,ms): #x PAN y LRHS_UP
#         z = torch.cat([x,y],dim=1)
#         res = y
#         ### Input stage
#         out1 = self.conv1_1(x)
...
#         ### Reconstruction stage
#         out = self.convlast(out3)* self.CA(ms) + res

#         return out


# version 5 use RWDB_new block*1 + sperctral attention + DAFB
# class MDANet(nn.Module):
#     def __init__(self,input_channel):
#         super(MDANet, self).__init__()
#         Dim=[1, input_channel+1, input_channel]
...
#         self.CA = CALayer(input_channel, 4, bias=True)
#         self.convlast = conv(nFeat, input_channel, 1, True)
#     def forward(self, x,y,ms): #x PAN y LRHS_UP
#         z = torch.cat([x,y],dim=1)
#         res = y
#         ### Input stage
